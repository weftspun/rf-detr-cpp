...
// decoder -> segmentation head) against the real RFDETRSegSmall checkpoint
// -- same encoder family as SegNano, num_windows=2 (vs SegNano's 1) and
// resolution 384 (grid 32). Same topk-override technique as
...
int main(int argc, char ** argv) {
    const char * backbone_gguf  = argc > 1 ? argv[1] : "models/rf-detr-seg-small-backbone.gguf";
    const char * projector_gguf = argc > 2 ? argv[2] : "models/rf-detr-seg-small-projector.gguf";
    const char * decoder_gguf   = argc > 3 ? argv[3] : "models/rf-detr-seg-small-decoder.gguf";
    const char * seg_gguf       = argc > 4 ? argv[4] : "models/rf-detr-seg-small-segmentation.gguf";
    const char * seg_ref_path   = argc > 5 ? argv[5] : "gen_reference/reference_segmentation_small.bin";
    ...
    dp.num_queries = 100; dp.num_classes = 91; dp.gw = 32; dp.gh = 32;

    SegmentationParams sp;
    sp.hidden_dim = 256; sp.num_blocks = 4; sp.downsample_ratio = 4; sp.image_w = 384; sp.image_h = 384;

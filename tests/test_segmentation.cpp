...
// decoder -> segmentation head) against the real RFDETRSegNano checkpoint.
// Uses the reference's own torch.topk indices as an override, same as
// test_decoder.cpp (see docs/decisions/decoder.md).
#include "backbone.h"
#include "decoder.h"
#include "projector.h"
...
int main(int argc, char ** argv) {
    const char * backbone_gguf  = argc > 1 ? argv[1] : "models/rf-detr-seg-nano-backbone.gguf";
    const char * projector_gguf = argc > 2 ? argv[2] : "models/rf-detr-seg-nano-projector.gguf";
    const char * decoder_gguf   = argc > 3 ? argv[3] : "models/rf-detr-seg-nano-decoder.gguf";
    const char * seg_gguf       = argc > 4 ? argv[4] : "models/rf-detr-seg-nano-segmentation.gguf";
    const char * seg_ref_path   = argc > 5 ? argv[5] : "gen_reference/reference_segmentation_nano.bin";
    ...
    if (!rfdetr_load(m, seg_gguf))       { fprintf(stderr, "failed to load %s\n", seg_gguf); return 1; }
    ...
    if (!read_ref(seg_ref_path, seg_ref, 5)) { fprintf(stderr, "failed to read %s\n", seg_ref_path); return 1; }
    ...
    ggml_tensor * final_mask = masks.back(); // (78,78,100,1)
    ...
    // Gate is looser than the other milestones' 5e-2 by design: mask logits
    // are a 256-channel dot product (spatial_features_proj . query_proj), and
    // this port's backbone+decoder outputs carry ~1e-3-level float drift
    // relative to upstream (see docs/decisions/segmentation.md for the
    // isolation tests confirming every sub-component matches near machine
    // precision when fed real captured inputs) -- a 256-term dot product
    // amplifies that by ~sqrt(256)=16x in the worst case, which is exactly
    // the scale of the residual max-abs-diff here (still tiny in mean:
    // ~2e-3). Not a logic bug; a property of a dot-product head over a
    // from-scratch reimplementation's inherent float non-bit-exactness.

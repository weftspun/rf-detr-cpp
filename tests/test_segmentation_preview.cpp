...
// decoder -> segmentation head) against the real RFDETRSegPreview
// checkpoint -- same encoder/window pattern and resolution as SegMedium
// (432, grid 36) but dec_layers=4 (not 5), num_queries=200. Same
// topk-override technique as test_segmentation.cpp (see
// docs/decisions/decoder.md). SegmentationParams::num_blocks MUST equal
// dec_layers -- see docs/decisions/0001-open-work.md's SegMedium note.
#include "backbone.h"
#include "decoder.h"
#include "projector.h"
...
int main(int argc, char ** argv) {
    const char * backbone_gguf  = argc > 1 ? argv[1] : "models/rf-detr-seg-preview-backbone.gguf";
    const char * projector_gguf = argc > 2 ? argv[2] : "models/rf-detr-seg-preview-projector.gguf";
    const char * decoder_gguf   = argc > 3 ? argv[3] : "models/rf-detr-seg-preview-decoder.gguf";
    const char * seg_gguf       = argc > 4 ? argv[4] : "models/rf-detr-seg-preview-segmentation.gguf";
    const char * seg_ref_path   = argc > 5 ? argv[5] : "gen_reference/reference_segmentation_preview.bin";

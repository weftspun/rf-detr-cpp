...
// against the real RFDETRMedium checkpoint -- same Nano-family backbone
// (patch_size==16, no bicubic interpolation) but resolution 576 (grid 36)
// and 4 decoder layers, confirming the existing code generalizes to this
// size with config-value changes only. Same topk-override technique as
// test_decoder.cpp/test_decoder_base.cpp/test_decoder_small.cpp (see
...
int main(int argc, char ** argv) {
    const char * backbone_gguf  = argc > 1 ? argv[1] : "models/rf-detr-medium-backbone.gguf";
    const char * projector_gguf = argc > 2 ? argv[2] : "models/rf-detr-medium-projector.gguf";
    const char * decoder_gguf   = argc > 3 ? argv[3] : "models/rf-detr-medium-decoder.gguf";
    const char * backbone_ref_path = argc > 4 ? argv[4] : "gen_reference/reference_backbone_medium.bin";
    const char * decoder_ref_path  = argc > 5 ? argv[5] : "gen_reference/reference_decoder_medium.bin";
    ...
    dp.num_queries = 300; dp.num_classes = 91; dp.gw = 36; dp.gh = 36;

...
// against the real RFDETRLarge (current, non-deprecated) checkpoint -- same
// Nano-family backbone (patch_size==16, no bicubic interpolation) but
// resolution 704 (grid 44) and 4 decoder layers. Distinct from
// RFDETRLargeDeprecated, which is patch_size==14/768-wide/2-feature-level
// and needs real new code (see docs/decisions/0001-open-work.md). Same
// topk-override technique as test_decoder.cpp/test_decoder_base.cpp/
// test_decoder_small.cpp/test_decoder_medium.cpp (see
...
int main(int argc, char ** argv) {
    const char * backbone_gguf  = argc > 1 ? argv[1] : "models/rf-detr-large-backbone.gguf";
    const char * projector_gguf = argc > 2 ? argv[2] : "models/rf-detr-large-projector.gguf";
    const char * decoder_gguf   = argc > 3 ? argv[3] : "models/rf-detr-large-decoder.gguf";
    const char * backbone_ref_path = argc > 4 ? argv[4] : "gen_reference/reference_backbone_large.bin";
    const char * decoder_ref_path  = argc > 5 ? argv[5] : "gen_reference/reference_decoder_large.bin";
    ...
    dp.num_queries = 300; dp.num_classes = 91; dp.gw = 44; dp.gh = 44;

    const int64_t res = 704;

// Isolated validation of src/deform_attn.cpp's ms_deform_attn_multilevel
// (num_levels=2) against the real upstream MSDeformAttn module on synthetic
// weights/inputs (gen_reference/gen_reference_deform_attn_multilevel.py) --
// same isolation-before-wiring-in discipline as test_deform_attn.cpp
// (num_levels=1), needed for RFDETRLargeDeprecated's P3+P5 projector_scale.
...
int main(int argc, char ** argv) {
    const char * ref_path = argc > 1 ? argv[1] : "gen_reference/reference_deform_attn_multilevel.bin";

    std::vector<NpyArray> ref;
    if (!read_ref(ref_path, ref, 12)) {
        fprintf(stderr, "failed to read %s (run gen_reference/gen_reference_deform_attn_multilevel.py first)\n", ref_path);
        return 1;
    }
    ...
    const int d_model = 8, n_heads = 2, n_points = 2;
    const std::vector<std::pair<int, int>> levels = { { 5, 5 }, { 3, 3 } }; // (gw,gh), matches the .py script
    int64_t total_tokens = 0;
    for (auto & lv : levels) total_tokens += (int64_t) lv.first * lv.second;
    ...
    ggml_tensor * value = ggml_new_tensor_3d(m.ctx_g, GGML_TYPE_F32, d_model, total_tokens, 1);
    ...
    ggml_tensor * out = ms_deform_attn_multilevel(m, query, value, refpts, "deform", n_heads, n_points, levels);

    bool ok = compute_cpu(m, out, 8192, [&] {

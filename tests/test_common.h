...
// Copied from see-through-cpp's tests/test_common.h (same methodology).
#pragma once
...
static ggml_backend_t rfdetr_backend_init() {
    const char * dev = getenv("RFDETR_DEVICE");
...
static bool rfdetr_load(Model & m, const char * path) {
    const char * dev = getenv("RFDETR_DEVICE");
    if (!(dev && strcmp(dev, "cpu") == 0)) {
        ggml_backend_dev_t d = ggml_backend_dev_by_type(GGML_BACKEND_DEVICE_TYPE_GPU);

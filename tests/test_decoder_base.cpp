...
// against the real RFDETRBase checkpoint -- the patch_size==14 (bicubic
// position-embed interpolation) + 3-decoder-layer variant, distinct from
// every Nano-family config validated so far. Same topk-override technique
// as test_decoder.cpp (see docs/decisions/decoder.md).
#include "backbone.h"
#include "decoder.h"
#include "projector.h"
...
int main(int argc, char ** argv) {
    const char * backbone_gguf  = argc > 1 ? argv[1] : "models/rf-detr-base-backbone.gguf";
    const char * projector_gguf = argc > 2 ? argv[2] : "models/rf-detr-base-projector.gguf";
    const char * decoder_gguf   = argc > 3 ? argv[3] : "models/rf-detr-base-decoder.gguf";
    const char * backbone_ref_path = argc > 4 ? argv[4] : "gen_reference/reference_backbone_base.bin";
    const char * decoder_ref_path  = argc > 5 ? argv[5] : "gen_reference/reference_decoder_base.bin";
    ...
    BackboneParams bp;
    bp.hidden = 384; bp.n_layer = 12; bp.n_head = 6; bp.patch_size = 14; bp.n_register = 0; bp.num_windows = 4;
    ...
    dp.num_queries = 300; dp.num_classes = 91; dp.gw = 40; dp.gh = 40;

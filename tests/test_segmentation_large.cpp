...
// decoder -> segmentation head) against the real RFDETRSegLarge
// checkpoint -- same encoder/window pattern as SegSmall/SegMedium,
// resolution 504 (grid 42), dec_layers=5, num_queries=200. Same
// topk-override technique as test_segmentation.cpp (see
// docs/decisions/decoder.md). SegmentationParams::num_blocks MUST equal
// dec_layers -- see docs/decisions/0001-open-work.md's SegMedium note for
// what happens when it doesn't (a large, real mask divergence, not the
// usual small amplified-float-drift residual).
#include "backbone.h"
#include "decoder.h"
#include "projector.h"
...
int main(int argc, char ** argv) {
    const char * backbone_gguf  = argc > 1 ? argv[1] : "models/rf-detr-seg-large-backbone.gguf";
    const char * projector_gguf = argc > 2 ? argv[2] : "models/rf-detr-seg-large-projector.gguf";
    const char * decoder_gguf   = argc > 3 ? argv[3] : "models/rf-detr-seg-large-decoder.gguf";
    const char * seg_gguf       = argc > 4 ? argv[4] : "models/rf-detr-seg-large-segmentation.gguf";
    const char * seg_ref_path   = argc > 5 ? argv[5] : "gen_reference/reference_segmentation_large.bin";
